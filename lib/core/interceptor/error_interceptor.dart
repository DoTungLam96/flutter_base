...
import 'package:base_flutter/l10n/l10n_export.dart';
  ...
  Future<ApiException> toApiException() async {
    ...
    if (errorCode == 503) {
      return ApiException.internalServerError(
          path, IBLocale.current.maintenance_message, '$errorCode', errorCode);
    }
    ...
    switch (errorCode) {
      case 400:
        return ApiException.badRequest(
            path,
            apiError.message ?? IBLocale.current.maintenance_message,
            apiError.code,
            errorCode);
      case 401:
      ...
      default:
        ...
        if (connectivity == ConnectivityResult.none) {
          return ApiException.noConnection(
              path, IBLocale.current.maintenance_message, '', 0);
        }
        return ApiException.noConnection(
            path, IBLocale.current.maintenance_message, '', 0);
    }
